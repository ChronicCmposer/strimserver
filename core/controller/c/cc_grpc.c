 ...
 *   1. Constants and helper utilities (I/O, time).
 *   2. HPACK: Huffman table + decode trie, integer coding, static table,
 *      dynamic table (4096-byte, eviction), block decoder, block encoder.
 *   3. HTTP/2 framing: frame write, incremental frame reader.
 *   4. Connection + stream state, flow control.
 *   5. Request building (HPACK request headers + gRPC DATA framing).
 *   6. Frame dispatch (SETTINGS/PING/WINDOW_UPDATE/DATA/HEADERS/
 *      CONTINUATION/RST_STREAM/GOAWAY), response processing, the gRPC
 *      message reassembler.
 *   7. Public API.
 *   8. Self-test (HPACK/HTTP2 unit vectors; runnable without a daemon).
 *
 * Philosophy (code-philosophy, 5 laws) as applied:
 *   - Early exit: every API function validates its handle/arguments first.
 *   - Parse don't validate: raw socket bytes are parsed into typed frame /
 *     header / message state at the boundary; the rest of the code only
 *     touches trusted state.
 *   - Atomic predictability: helpers are pure (buffer in, bytes out, or a
 *     counted return); no hidden global mutation except the explicitly
 *     owned per-connection state.
 *   - Fail fast: any protocol violation (bad HPACK, bad frame, window
 *     overflow, unknown stream) aborts the connection with a descriptive
 *     CC_GRPC_ERR_* code — never a silent continuation.
 *   - Intentional naming: hpack_* / frame_* / stream_* / conn_* prefixes
 *     read like English; no single-letter names except loop indexes.
 *
 * Single-threaded by design: one connection handle is driven from one
 * thread. Callbacks must not re-enter the library.
 *
...
/* The generated containerd codecs (SubscribeRequest construction; the
 * protocol layer itself stays protobuf-agnostic except for this one
 * message, which is part of the Subscribe API surface). */
...
#define CC_MAX_CONNS       4
#define CC_MAX_STREAMS     8
#define CC_MSG_RING        4 /* retained grpc-message texts */
#define CC_HDR_BLOCK_MAX   (64 * 1024)
#define CC_HAND_SHAKE_MS   5000
#define CC_UNARY_SLICE_MS  50
#define CC_POLL_FRAME_BUDGET 128
#define CC_HUFF_NODES      1024
#define CC_MAX_FIELDS      32
#define CC_REQ_HDR_BLOCK   1024
#define CC_DATA_BUF        (16384 + 5)
...
#define HPACK_DYN_MAX 128 /* 4096 / min entry size 32 */

/* =========================================================================
 * Utilities: blocking I/O with MSG_NOSIGNAL, monotonic time
 * ========================================================================= */
...
static int64_t deadline_from_ms(int64_t ms) { return mono_ms() + ms; }
...
/* =========================================================================
 * HPACK: Huffman table (generated from golang.org/x/net/http2/hpack's
 * tables.go, which mirrors RFC 7541 Appendix B; entry 256 is EOS)
 * ========================================================================= */
...
static struct huff_node g_huff_nodes[CC_HUFF_NODES];
static int g_huff_nnodes;
static int g_huff_built;

/* Build the decode trie once. Single-threaded controller: a plain flag is
 * the correct guard. */
static void huff_build(void) {
  ...
  for (i = 0; i < 257; i++) {
    ...
    for (b = bits - 1; b >= 0; b--) {
      ...
      if (next < 0) {
        if (g_huff_nnodes >= CC_HUFF_NODES) {
          g_huff_built = 2;
          return;
        }
        next = g_huff_nnodes++;
...
static int huff_decode(const uint8_t *in, size_t in_len, uint8_t *out,
  ...
  if (node != 0) {
    /* Trailing bits fill the final byte: at most 7 padding bits, and they
     * must be a prefix of EOS (all ones). Overlong or non-ones padding is
     * a COMPRESSION_ERROR (matches RFC 7541 §5.2 and grpc-go). */
    if (!pad_ones || depth > 7)
      return -1;
  }
  *out_len = emitted;
...
/* Decode an integer with `prefix` prefix bits. Returns bytes consumed, or
 * -1 on error. */
...
/* Encode an integer with `prefix` prefix bits, ORing `pattern` (the HPACK
 * field-type bits, e.g. 0x80 indexed / 0x40 incremental) into the first
 * byte. Returns bytes written, or 0 if it does not fit. */
...
/* Encode a bare integer (pattern 0). */
...
struct hpack_dyn {
  struct hpack_dyn_entry entries[HPACK_DYN_MAX];
  size_t count;
  size_t size;
  size_t max_size; /* current table size limit */
};
...
static void dyn_add(struct hpack_dyn *dyn, const uint8_t *name, size_t name_len,
  ...
  if (entry_size > dyn->max_size)
    return;
  while (dyn->size + entry_size > dyn->max_size || dyn->count >= HPACK_DYN_MAX) {
    if (dyn->count == 0)
      return;
    dyn_evict_one(dyn);
  }
...
/* =========================================================================
 * HPACK: string coding (RFC 7541 §5.2). Requests are sent plain (H=0);
 * responses may be Huffman-coded (H=1) — both are decoded.
 * ========================================================================= */
...
/* Resolve a name index into a field's name. index==0 is not a lookup. */
...
/* Decode a complete header block into `fields`. Returns 0 on success, -1 on
 * protocol error (a COMPRESSION_ERROR per RFC 7541 §4.3). */
static int hpack_decode_block(struct hpack_dyn *dyn, const uint8_t *in,
  ...
  while (off < in_len) {
    ...
    return -1; /* reserved pattern (should be unreachable) */
  }
  *n_fields = n;
  return 0;
}
...
static size_t hpack_encode_field(struct hpack_dyn *dyn, uint8_t *out,
  ...
  if (name_idx != 0) {
    /* Literal with incremental indexing, indexed name: RFC 7541 §6.2.1
     * uses a 6-bit prefix (01 + Index). */
...
struct frame_reader {
  ...
  int hdr_fill;
  uint8_t *payload;
  size_t payload_cap; /* allocated capacity */
  size_t payload_len; /* current frame's payload length */
  size_t payload_fill;
  uint8_t type;
  uint8_t flags;
  uint32_t sid;
...
static int frame_reader_fill(struct frame_reader *r, int fd) {
  for (;;) {
    ...
    {
      /* Read only this frame's payload bytes — never past payload_len,
       * even though the buffer may be larger (reused from a previous
       * frame). */
...
enum stream_state {
  ...
  SS_DONE = 2,    /* finished: trailers seen, or failed/cancelled */
};

struct cc_stream {
  uint32_t id;
  int in_use;
  int type;      /* ST_UNARY or ST_SUBSCRIBE */
  int state;     /* stream_state */
  int status;    /* final gRPC status when state == SS_DONE */
  int err;       /* client-side CC_GRPC_ERR_* when we aborted locally */
  int32_t recv_window; /* server -> client, we advertise */
  int32_t send_window; /* client -> server, server advertises */

  /* gRPC message reassembler: 0 = need 5-byte header, 1 = need message
   * bytes (rasm_state is reset to 0 when a message completes). */
  int rasm_state;
  uint8_t rasm_hdr[5];
  int rasm_hdr_fill;
  uint32_t rasm_msglen;
  uint32_t rasm_got;
  uint8_t *rasm_buf; /* subscribe: per-message buffer */
  size_t rasm_buf_cap;
  int msg_count; /* complete messages received on this stream */

  void *cb_ctx;
  void (*cb)(void *cb_ctx, const uint8_t *env, uint32_t len);

  uint8_t *resp; /* unary destination (valid while the unary blocks) */
  uint32_t resp_cap;
  uint32_t resp_len;
};
...
struct cc_conn {
  ...
  int ready;
  int dead;
  int dead_code; /* CC_GRPC_EV_* that killed the connection */
  char namespace_[CC_GRPC_MAX_NAMESPACE];
  uint64_t unary_timeout_ns;

  uint32_t next_sid;
  struct cc_stream streams[CC_MAX_STREAMS];
  ...
  struct hpack_dyn dyn_enc; /* request encoder's table (server decodes it) */
  struct hpack_dyn dyn_dec; /* response decoder's table (server encodes it) */

  struct frame_reader rx;

  uint8_t *hdr_block; /* HEADERS + CONTINUATION assembly */
  ...
  char msg_ring[CC_MSG_RING][CC_GRPC_MAX_HDR_MSG];
  int msg_status[CC_MSG_RING];
  int msg_head;

  int ev_flags;
};

static struct cc_conn g_conns[CC_MAX_CONNS];

#define EVF_MSG        0x1
#define EVF_STREAM_END 0x2

/* =========================================================================
 * Internal helpers
 * ========================================================================= */

static void stream_destroy(struct cc_conn *c, struct cc_stream *s);

static struct cc_conn *conn_from_handle(int h) {
  if (h < 1 || h > CC_MAX_CONNS)
    return NULL;
  if (!g_conns[h - 1].in_use)
    return NULL;
  return &g_conns[h - 1];
}

static struct cc_stream *stream_find(struct cc_conn *c, uint32_t sid) {
  int i;
  for (i = 0; i < CC_MAX_STREAMS; i++) {
...
static struct cc_stream *stream_alloc(struct cc_conn *c, int type) {
  int i;
  int done_slot = -1;
  for (i = 0; i < CC_MAX_STREAMS; i++) {
    if (!c->streams[i].in_use) {
      struct cc_stream *s = &c->streams[i];
      memset(s, 0, sizeof(*s));
      s->in_use = 1;
      s->type = type;
      s->id = c->next_sid;
  ...
  if (done_slot >= 0) {
    struct cc_stream *s = &c->streams[done_slot];
    stream_destroy(c, s);
...
static void conn_store_message(struct cc_conn *c, int status,
                               const char *msg) {
  if (msg == NULL || msg[0] == '\0')
    msg = "";
  c->msg_head = (c->msg_head + 1) % CC_MSG_RING;
  c->msg_status[c->msg_head] = status;
  snprintf(c->msg_ring[c->msg_head], CC_GRPC_MAX_HDR_MSG, "%s", msg);
}

/* Percent-decode a gRPC message (grpc-go percent-encodes grpc-message).
 * The input is length-delimited (not NUL-terminated). */
...
static void complete_all_streams(struct cc_conn *c, int status,
                                 const char *msg) {
  int i;
  for (i = 0; i < CC_MAX_STREAMS; i++) {
    struct cc_stream *s = &c->streams[i];
  ...
  c->ev_flags |= EVF_STREAM_END;
}

static void conn_fail(struct cc_conn *c, int ev_code) {
  if (c->dead)
    return;
  c->dead = 1;
  c->dead_code = ev_code;
  complete_all_streams(c, CC_GRPC_STATUS_UNAVAILABLE, "connection failed");
}

/* Abort a stream with RST_STREAM and mark it finished (protocol error). */
static void stream_proto_error(struct cc_conn *c, struct cc_stream *s,
                               uint32_t h2_code) {
  frame_write_rst(c->fd, s->id, h2_code);
  s->state = SS_DONE;
  s->status = CC_GRPC_STATUS_INTERNAL;
  s->err = 0;
  free(s->rasm_buf);
  s->rasm_buf = NULL;
  s->rasm_buf_cap = 0;
...
static int handle_settings(struct cc_conn *c, const uint8_t *p, size_t len) {
  size_t i;
  if (len % 6 != 0)
    return -1;
  for (i = 0; i < len; i += 6) {
...
static int handle_header_block(struct cc_conn *c, uint32_t sid,
                               const uint8_t *block, size_t len,
                               uint8_t flags);
static void stream_feed_data(struct cc_conn *c, struct cc_stream *s,
                             const uint8_t *data, size_t len);

static int handle_headers_frame(struct cc_conn *c, uint8_t flags,
...
static int handle_continuation_frame(struct cc_conn *c, uint8_t flags,
                                     const uint8_t *payload, size_t len) {
  if (!c->hdr_block_pending)
    return -1;
  if (c->hdr_block_len + len > CC_HDR_BLOCK_MAX)
    return -1;
...
static int handle_header_block(struct cc_conn *c, uint32_t sid,
                               const uint8_t *block, size_t len,
                               uint8_t flags) {
  struct hpack_field fields[CC_MAX_FIELDS];
  int n_fields = 0;
  size_t block_len;
  ...
  if (has_status && !status_ok && !has_grpc_status) {
    /* Non-200 without a gRPC status: transport-level failure. */
    grpc_status = (status_value == 404) ? CC_GRPC_STATUS_UNIMPLEMENTED
                                        : CC_GRPC_STATUS_INTERNAL;
    has_grpc_status = 1;
  }
  if (!has_grpc_status && !content_type_ok) {
    /* Initial headers without a gRPC content type: not a gRPC response. */
    grpc_status = CC_GRPC_STATUS_INTERNAL;
    has_grpc_status = 1;
  }
  if (!encoding_ok) {
    grpc_status = CC_GRPC_STATUS_INTERNAL;
...
static int handle_data_frame(struct cc_conn *c, uint8_t flags, uint32_t sid,
                             const uint8_t *payload, size_t len) {
  struct cc_stream *s = stream_find(c, sid);
  size_t data_off = 0;
  size_t data_len = len;
  ...
  if (len > 0) {
    frame_write_window_update(c->fd, sid, (uint32_t)len);
    frame_write_window_update(c->fd, 0, (uint32_t)len);
    c->conn_recv_window += (int32_t)len;
    s->recv_window += (int32_t)len;
  }
  ...
  if ((flags & H2_FLAG_END_STREAM) != 0) {
    if (s->rasm_state != 0) {
      stream_proto_error(c, s, H2_ERR_PROTOCOL);
      return 0;
    }
  }
  return 0;
  ...
  while (off < len) {
    if (s->rasm_state == 0) {
      ...
      if (s->rasm_hdr_fill < 5)
        return;
      if (s->rasm_hdr[0] != 0) {
        stream_proto_error(c, s, H2_ERR_INTERNAL); /* compressed frame */
        return;
      }
      ...
      if (s->rasm_msglen > CC_GRPC_MAX_MSG) {
        stream_proto_error(c, s, H2_ERR_FRAME_SIZE);
        return;
      }
      s->rasm_got = 0;
      if (s->type == ST_SUBSCRIBE) {
        ...
        if (nb == NULL) {
          stream_proto_error(c, s, H2_ERR_INTERNAL);
          return;
        }
        s->rasm_buf = nb;
        s->rasm_buf_cap = cap;
      } else if (s->resp != NULL && s->rasm_msglen > s->resp_cap) {
        /* Response does not fit the caller's buffer: abort the stream so
         * the server stops sending, drain subsequent frames. */
        frame_write_rst(c->fd, s->id, H2_ERR_CANCEL);
        s->state = SS_DONE;
        s->err = CC_GRPC_ERR_TOOBIG;
        s->status = 0;
        c->ev_flags |= EVF_STREAM_END;
        return;
      }
    ...
    if (s->rasm_state == 1) {
      ...
      if (s->rasm_got >= s->rasm_msglen) {
        if (s->type == ST_SUBSCRIBE) {
          s->cb(s->cb_ctx, s->rasm_buf, s->rasm_msglen);
          c->ev_flags |= EVF_MSG;
        } else {
...
static int handle_window_update(struct cc_conn *c, uint32_t sid,
                                const uint8_t *payload, size_t len) {
  uint32_t inc;
  if (len != 4)
    return -1;
...
static int handle_goaway(struct cc_conn *c, const uint8_t *payload,
                         size_t len) {
  uint32_t last_sid;
  if (len < 8)
    return -1;
  ...
  complete_all_streams(c, CC_GRPC_STATUS_UNAVAILABLE, "server GOAWAY");
  c->dead = 1;
  c->dead_code = CC_GRPC_EV_GOAWAY;
  return 0;
}

static int handle_rst_stream(struct cc_conn *c, uint32_t sid,
                             const uint8_t *payload, size_t len) {
  struct cc_stream *s;
  if (len != 4)
    return -1;
  ...
  s->state = SS_DONE;
  s->status = CC_GRPC_STATUS_CANCELLED;
  s->err = 0;
  free(s->rasm_buf);
  s->rasm_buf = NULL;
  s->rasm_buf_cap = 0;
...
static int handle_ping(struct cc_conn *c, uint8_t flags,
                       const uint8_t *payload, size_t len) {
...
static int process_one_frame(struct cc_conn *c) {
  struct frame_reader *r = &c->rx;
  int fr = frame_reader_fill(r, c->fd);
  int rc;

  if (fr < 0)
    return CC_GRPC_ERR_IO;
  if (fr == 0)
    return 0;
...
static int conn_poll_once(struct cc_conn *c, int timeout_ms) {
  struct pollfd pfd;
  int pr;
  int budget = CC_POLL_FRAME_BUDGET;
  ...
  if (pr < 0) {
    if (errno == EINTR)
      return CC_GRPC_EV_NONE;
    conn_fail(c, CC_GRPC_EV_IO);
    return CC_GRPC_EV_IO;
  }
  if (pr == 0)
    return CC_GRPC_EV_NONE;
  ...
  while (budget-- > 0) {
    ...
    if (r < 0) {
      conn_fail(c, (r == CC_GRPC_ERR_PROTO) ? CC_GRPC_EV_PROTO
                                            : CC_GRPC_EV_IO);
      return c->dead_code;
    }
    if (r == 0)
  ...
  if (c->dead)
    return c->dead_code;
  if (c->ev_flags & EVF_MSG)
    return CC_GRPC_EV_MSG;
  if (c->ev_flags & EVF_STREAM_END)
    return CC_GRPC_EV_STREAM_END;
  return CC_GRPC_EV_NONE;
}
...
/* The 24-byte HTTP/2 connection preface (exactly 24 bytes, no NUL). */
...
static int format_grpc_timeout(uint64_t ns, char *out, size_t cap) {
  struct {
    char unit;
    uint64_t div;
  } units[] = {{'H', 3600000000000ULL}, {'M', 60000000000ULL},
               {'S', 1000000000ULL},     {'m', 1000000ULL},
               {'u', 1000ULL},           {'n', 1ULL}};
  size_t i;
  for (i = 0; i < sizeof(units) / sizeof(units[0]); i++) {
    uint64_t v = ns / units[i].div;
    if (v <= 99999999ULL) {
      int n = snprintf(out, cap, "%llu%c", (unsigned long long)v,
                       units[i].unit);
...
static int build_request_headers(struct cc_conn *c, const char *path,
  ...
  if (w == 0)
    return -1;
  n += w;
  if (c->unary_timeout_ns > 0) {
    if (format_grpc_timeout(c->unary_timeout_ns, timeout_buf,
                            sizeof(timeout_buf)) < 0)
      return -1;
    w = hpack_encode_field(&c->dyn_enc, out + n, cap - n, "grpc-timeout",
                           timeout_buf);
    if (w == 0)
      return -1;
    n += w;
 ...
 * sent). Returns 0 or a negative CC_GRPC_ERR_*. */
static int send_request(struct cc_conn *c, struct cc_stream *s,
                        const char *path, const uint8_t *req, uint32_t req_len) {
  uint8_t hdr_block[CC_REQ_HDR_BLOCK];
  size_t hdr_len = 0;
  uint8_t grpc_hdr[5];
  uint32_t frame_max = c->srv_max_frame;
  uint32_t total = req_len + 5; /* gRPC header + body */
  uint32_t sent = 0;
  uint8_t buf[CC_DATA_BUF];
  ...
  while (sent < total) {
    ...
    if (s->send_window < (int32_t)chunk || c->conn_send_window < (int32_t)chunk)
      return CC_GRPC_ERR_PROTO; /* would exceed the server's send window */
...
int cc_grpc_init(const char *sock_path, const char *namespace_) {
  ...
  if (strlen(path) >= CC_GRPC_MAX_SOCK_PATH)
    return 0;
  if (strlen(ns) >= CC_GRPC_MAX_NAMESPACE)
    return 0;

  for (i = 0; i < CC_MAX_CONNS; i++) {
    if (!g_conns[i].in_use) {
      c = &g_conns[i];
      break;
    }
  ...
  c->in_use = 1;
  c->fd = fd;
  /* The frame reader relies on EAGAIN to know when a frame is partial, so
   * the socket must be non-blocking; poll() drives the waits. */
  {
  ...
  c->msg_head = -1;
  strncpy(c->namespace_, ns, CC_GRPC_MAX_NAMESPACE - 1);
  c->namespace_[CC_GRPC_MAX_NAMESPACE - 1] = '\0';
  dyn_reset(&c->dyn_enc);
  dyn_reset(&c->dyn_dec);

  /* Our SETTINGS payload: ENABLE_PUSH=0, MAX_CONCURRENT_STREAMS=100,
  ...
  /* Connection preface, then SETTINGS — the FIRST frame after the preface
   * MUST be SETTINGS (grpc-go rejects anything else). */
  ...
  /* Wait for the server's SETTINGS (grpc-go sends it immediately); ACK it
   * and absorb any PING / WINDOW_UPDATE that arrive during the handshake. */
  deadline = deadline_from_ms(CC_HAND_SHAKE_MS);
  while (!c->ready && !c->dead) {
    ...
    conn_poll_once(c, (int)(left > 100 ? 100 : left));
  }
  if (c->dead) {
    close(fd);
    c->in_use = 0;
...
int cc_grpc_unary(int h, const char *method, const uint8_t *req,
                  ...
                  uint32_t *resp_len) {
  struct cc_conn *c = conn_from_handle(h);
  struct cc_stream *s;
  int64_t deadline;
  uint32_t timeout_ms;
  int rc;
  ...
  if (c == NULL || method == NULL || resp_len == NULL)
    return CC_GRPC_ERR_BADARG;
  if (c->dead)
    return (c->dead_code == CC_GRPC_EV_GOAWAY) ? CC_GRPC_ERR_GOAWAY
                                               : CC_GRPC_ERR_IO;
  if (resp == NULL && resp_cap != 0)
    return CC_GRPC_ERR_BADARG;
  if (req_len > CC_GRPC_MAX_MSG)
    return CC_GRPC_ERR_TOOBIG;

  s = stream_alloc(c, ST_UNARY);
  if (s == NULL)
    return CC_GRPC_ERR_NOSTREAM;
  s->resp = resp;
  s->resp_cap = resp_cap;
  s->resp_len = 0;
  ...
  timeout_ms = (c->unary_timeout_ns > 0)
                   ? (uint32_t)(c->unary_timeout_ns / 1000000ULL)
                   : CC_GRPC_DEFAULT_UNARY_TIMEOUT_MS;
  deadline = deadline_from_ms(timeout_ms);

  while (s->state != SS_DONE) {
    if (time_left_ms(deadline) <= 0) {
      stream_destroy(c, s);
      return CC_GRPC_ERR_TIMEOUT;
    }
    conn_poll_once(c, CC_UNARY_SLICE_MS);
    if (c->dead) {
      /* The RPC may have completed in the same frame batch before the
       * connection died (e.g. trailers + server RST + close): honor the
       * completed stream and surface its status. */
      if (s->state == SS_DONE)
        break;
      stream_destroy(c, s);
      if (c->dead_code == CC_GRPC_EV_GOAWAY)
        return CC_GRPC_ERR_GOAWAY;
      if (c->dead_code == CC_GRPC_EV_PROTO)
        return CC_GRPC_ERR_PROTO;
      return CC_GRPC_ERR_IO;
    }
  }
...
int cc_grpc_subscribe(int h, const char *filter, void *cb_ctx,
  ...
  struct cc_conn *c = conn_from_handle(h);
  struct cc_stream *s;
  Containerd__Services__Events__V1__SubscribeRequest req =
      CONTAINERD__SERVICES__EVENTS__V1__SUBSCRIBE_REQUEST__INIT;
  char *filters[1];
  uint8_t body[512];
  size_t body_len = 0;
  const char *method = "/containerd.services.events.v1.Events/Subscribe";
  int rc;
  int live = 0;
  int i;

  if (c == NULL || filter == NULL || cb == NULL)
    return 0;
  if (c->dead)
    return 0;
  ...
  if ((uint32_t)live >= c->srv_max_streams)
    return 0;

  filters[0] = (char *)filter;
  req.n_filters = 1;
  req.filters = filters;
  body_len =
      containerd__services__events__v1__subscribe_request__get_packed_size(&req);
  if (body_len > sizeof(body))
    return 0;
  containerd__services__events__v1__subscribe_request__pack(&req, body);
  ...
  rc = send_request(c, s, method, body, (uint32_t)body_len);
  if (rc < 0) {
    stream_destroy(c, s);
    return 0;
  }
...
int cc_grpc_poll(int h, int timeout_ms) {
  ...
  if (c == NULL)
    return CC_GRPC_ERR_BADARG;
  return conn_poll_once(c, timeout_ms);
}

int cc_grpc_cancel(int h, int stream_id) {
  struct cc_conn *c = conn_from_handle(h);
  struct cc_stream *s;
  if (c == NULL || stream_id <= 0)
    return CC_GRPC_ERR_BADARG;
  if (c->dead)
    return CC_GRPC_ERR_NOCONN;
  s = stream_find(c, (uint32_t)stream_id);
  if (s == NULL)
    return CC_GRPC_ERR_NOSTREAM;
  if (s->state != SS_DONE) {
    if (frame_write_rst(c->fd, s->id, H2_ERR_CANCEL) < 0)
      return CC_GRPC_ERR_IO;
    s->state = SS_DONE;
    s->status = CC_GRPC_STATUS_CANCELLED;
    s->err = CC_GRPC_ERR_CANCEL;
  }
  return 0;
}
...
int cc_grpc_errstr(int h, int grpc_status, char *buf, uint32_t cap) {
  ...
  if (c->msg_head < 0)
    return -1;
  for (i = 0; i < CC_MSG_RING; i++) {
    int slot = (c->msg_head - i + CC_MSG_RING * 2) % CC_MSG_RING;
    if (c->msg_status[slot] < 0)
      continue;
    if (grpc_status == 0 || c->msg_status[slot] == grpc_status) {
      snprintf(buf, cap, "%s", c->msg_ring[slot]);
      return c->msg_status[slot];
    }
  }
  return -1;
}
...
const char *cc_grpc_version(void) {
  return "cc_grpc 1.0 (h2c containerd client)";
}
...
static int selftest_dynamic(void) {
        ...
        "dyn pair miss", &failures);
  {
    ...
    check(dyn.count <= HPACK_DYN_MAX, "dyn count bounded", &failures);
...
static int selftest_rfc_block(void) {
  ...
  memset(&dyn, 0, sizeof(dyn));
  dyn_reset(&dyn);
  if (hpack_decode_block(&dyn, block, sizeof(block), fields, CC_MAX_FIELDS,
                         &n) < 0) {
...
static int selftest_request_headers(void) {
  int failures = 0;
  struct cc_conn c;
  uint8_t block[1024];
  size_t blen = 0;
  ...
  memset(&c, 0, sizeof(c));
  strncpy(c.namespace_, "default", sizeof(c.namespace_) - 1);
  dyn_reset(&c.dyn_enc);
  dyn_reset(&c.dyn_dec);

  if (build_request_headers(
          &c, "/containerd.services.version.v1.Version/Version", block,
          sizeof(block), &blen) < 0) {
    check(0, "req headers encode", &failures);
    return failures;
  }
  if (hpack_decode_block(&c.dyn_enc, block, blen, fields, CC_MAX_FIELDS, &n) < 0) {
    check(0, "req headers decode", &failures);
    return failures;
  }
  for (i = 0; i < n; i++) {
    if (header_name_eq(&fields[i], ":method"))
      check(header_value_eq(&fields[i], "POST"), "req :method", &failures);
    else if (header_name_eq(&fields[i], ":scheme"))
      check(header_value_eq(&fields[i], "http"), "req :scheme", &failures);
    else if (header_name_eq(&fields[i], ":path"))
      check(header_value_eq(
                &fields[i], "/containerd.services.version.v1.Version/Version"),
            "req :path", &failures);
    else if (header_name_eq(&fields[i], ":authority"))
      check(header_value_eq(&fields[i], "localhost"), "req :authority",
            &failures);
    else if (header_name_eq(&fields[i], "content-type"))
      check(header_value_eq(&fields[i], "application/grpc"), "req content-type",
            &failures);
    else if (header_name_eq(&fields[i], "te"))
      check(header_value_eq(&fields[i], "trailers"), "req te", &failures);
    else if (header_name_eq(&fields[i], "user-agent"))
      check(header_value_eq(&fields[i], "grpc-c/1.0"), "req user-agent",
            &failures);
    else if (header_name_eq(&fields[i], "containerd-namespace"))
      check(header_value_eq(&fields[i], "default"), "req namespace", &failures);
  }
  hpack_free_fields(fields, n);

  {
    uint8_t block2[1024];
    size_t blen2 = 0;
    struct hpack_field fields2[CC_MAX_FIELDS];
    int n2 = 0;
    int j;
    if (build_request_headers(
            &c, "/containerd.services.containers.v1.Containers/List", block2,
            sizeof(block2), &blen2) < 0) {
      check(0, "req2 encode", &failures);
      return failures;
    }
    check(blen2 < blen, "dyn table shrinks block", &failures);
    if (hpack_decode_block(&c.dyn_enc, block2, blen2, fields2, CC_MAX_FIELDS,
                           &n2) < 0) {
      check(0, "req2 decode", &failures);
      return failures;
    }
    for (j = 0; j < n2; j++) {
      if (header_name_eq(&fields2[j], ":path"))
        check(header_value_eq(
                  &fields2[j],
                  "/containerd.services.containers.v1.Containers/List"),
              "req2 :path", &failures);
      else if (header_name_eq(&fields2[j], "containerd-namespace"))
        check(header_value_eq(&fields2[j], "default"), "req2 namespace",
              &failures);
    }
    hpack_free_fields(fields2, n2);
  }
  dyn_reset(&c.dyn_enc);
  dyn_reset(&c.dyn_dec);
  return failures;
...
int cc_grpc_selftest(int verbose) {
  ...
  n = selftest_huffman();
  if (verbose)
    printf("SELFTEST huffman: %s\n", n ? "FAIL" : "PASS");
  failures += n;

  n = selftest_integer();
  if (verbose)
    printf("SELFTEST integer: %s\n", n ? "FAIL" : "PASS");
  failures += n;

  n = selftest_static();
  if (verbose)
    printf("SELFTEST static-table: %s\n", n ? "FAIL" : "PASS");
  failures += n;

  n = selftest_dynamic();
  if (verbose)
    printf("SELFTEST dynamic-table: %s\n", n ? "FAIL" : "PASS");
  failures += n;

  n = selftest_rfc_block();
  if (verbose)
    printf("SELFTEST rfc7541-block: %s\n", n ? "FAIL" : "PASS");
  failures += n;

  n = selftest_request_headers();
  if (verbose)
    printf("SELFTEST request-headers: %s\n", n ? "FAIL" : "PASS");
  failures += n;

  n = selftest_frame_roundtrip();
  if (verbose)
    printf("SELFTEST frame-io: %s\n", n ? "FAIL" : "PASS");
  failures += n;

  if (verbose)
    printf("SELFTEST total: %d failures\n", failures);
  return failures;
}