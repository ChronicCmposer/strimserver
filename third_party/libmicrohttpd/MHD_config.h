...
#define PACKAGE_STRING "GNU libmicrohttpd 1.0.10"
...
/* Bazel vendoring patch (not emitted by this host's configure run):
   stdbool.h is a compiler-provided header on both target arches (glibc
   amd64/arm64 with gcc/clang), so `bool` is always available. */
#define HAVE_STDBOOL_H 1

/* Bazel vendoring patch: upstream configure passes -D_GNU_SOURCE on the
   command line via MHD_SYS_EXT (m4/mhd_sys_extentions.m4); folded into the
   committed header because it is included before any system header.
   Required for accept4(), pthread_setname_np(), etc. on glibc. */
