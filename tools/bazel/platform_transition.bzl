...
name (strimserver-deployment.unchecked.tar) in BOTH configurations, so the
...
platform_transition = rule(
    ...
    attrs = {
        ...
        "out": attr.string(
            mandatory = True,
            doc = "The output basename (e.g. strimserver-deployment-arm64.unchecked.tar); must differ from the target's own basename to avoid runfiles collisions.",
        ),
